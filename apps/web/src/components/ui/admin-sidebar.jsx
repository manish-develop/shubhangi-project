...
export const MobileSidebar = ({ className, children, ...props }) => {
	...
	return (
		<div className={cn('h-14 px-4 py-3 flex flex-row md:hidden items-center justify-between bg-primary text-primary-foreground w-full')} {...props}>
			...
			<AnimatePresence>
				{open && (
					<motion.div
						...
						className={cn('fixed h-full w-full inset-0 bg-primary text-primary-foreground p-10 z-[100] flex flex-col justify-between', className)}
					>
...
export const SidebarLink = ({ link, className, ...props }) => {
	...
	return (
		<Link to={link.href} className={cn('flex items-center justify-start gap-3 group/sidebar py-2.5 px-2 rounded-lg', className)} {...props}>
			{link.icon}
			<motion.span
				animate={{
				...
				className="text-sm whitespace-pre inline-block !p-0 !m-0 transition duration-150 group-hover/sidebar:translate-x-1"
			>
				{link.label}
			</motion.span>
		</Link>
